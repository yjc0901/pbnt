...
app.use(helmet({
    ...
    //     },
    // },
    contentSecurityPolicy: false,  // 필요한 경우 CSP 비활성화 (특히 개발 환경에서)
...
// === 비밀번호 해싱 (암호화할 때 한 번만 실행시키고 주석 or 삭제) ===
// async function migratePasswords() {
//     const users = await asyncQuery('SELECT id, password FROM users');
...
app.listen(PORT, "0.0.0.0", () => {
    ...
    // 비밀번호 마이그레이션 실행 (암호화할 때 한 번만 실행시키고 주석 or 삭제)
    // (async () => {
    //     try {
    //         console.log("비밀번호 암호화 마이그레이션 시작...");
    //         await migratePasswords();
    //         console.log("비밀번호 암호화 마이그레이션 완료!");
    //     } catch (err) {
    //         console.error("비밀번호 마이그레이션 중 오류 발생:", err);
    //     }
    // })();
});
